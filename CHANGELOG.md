...
### [2026-07-07] コミット前検証手順にE2Eテストの実行を明記した
* **修正の動機・概要**:
  - 「コピー中にフォルダカード・ドロップゾーンの操作が見た目上も無効化されるようにした」（本ファイル直下参照）の対応時、`FolderCard`のクリック・ナビゲーションを検証する`src/tests/e2e.spec.ts`が存在するにもかかわらず、`npm run test:e2e`を実行せずにコミットしようとしていたことをユーザーから指摘された。
  - 原因を調査したところ、`commit_rules.md`の「開発プロジェクト固有の検証手順」には`test:unit`・`lint`・`typecheck`のみが必須コマンドとして記載されており、`test:e2e`が抜けていたことが判明した（`AGENTS.md`や`issue-implement`スキルの「E2Eテストはtest_rules.mdに従う」という記載は、テストの書き方のルールへの参照であり、コミット前の実行必須化までは規定していなかった）。
  - 再発防止のため、`commit_rules.md`にE2Eテストの実行を明記した。ビルドを伴い実行コストが大きいため、`src/`・`electron/`配下の実装ファイルに修正がある場合のみ必須とし、ドキュメントのみの修正では不要という条件を付けた。
* **各ファイルへの影響と変更内容**:
  * **実装**: `commit_rules.md`の絶対遵守ルール2「テスト・リント・型チェックの無視禁止」に「対象範囲に応じたE2Eテスト」を明記。「開発プロジェクト固有の検証手順」に`npm run test:e2e`（`src/`・`electron/`配下の実装ファイルに修正がある場合は必須、ドキュメントのみの修正では不要）を追加。
  * **README.md / 仕様書**: 該当なし（`commit_rules.md`はコミット規約であり、README.md・`specs/`配下の仕様書とは扱う対象が異なるため乖離は発生していない）。
  * コードの変更を伴わないため、`npm run test:unit`等の実行は不要（本ルール自体の条件に基づき、ドキュメントのみの修正としてE2Eテストも実行不要）。
...
* **各ファイルへの影響と変更内容**:
  - **`README.md`**: `test_rules.md`・`commit_rules.md`への`file:///Users/fujiwalatex/...`絶対リンクを、相対リンク（`./test_rules.md`、`./commit_rules.md`）に修正。
  - **`.agents/skills/auto-commit/SKILL.md`**: `commit_rules.md`への絶対リンクを相対リンク（`../../../commit_rules.md`）に修正。
  - **`specs/auto_commit_skill_specification.md`**: ディレクトリ構成図のルート表記を、絶対パスから`./ (プロジェクトルート)`という環境非依存の表現に修正。
  - **rules.md**: 「ドキュメント・設定ファイルに特定PCのフルパス（絶対パス）を書かない」ルールを追加。
  - **実装 / テスト**: 変更なし（ドキュメントのみの修正）。
  - コードの変更を伴わないため、`npm run test:unit`等の実行は不要。`grep`で`/Users/`を含む記述がリポジトリ内に残っていないことを確認済み。

### [2026-07-06] 動画プレビューのカクつき軽減としてautoPlayを無効化した
* **修正の動機・概要**:
  - プレビューモーダルで動画を再生するとコマ落ち・カクつきが発生するとユーザーから報告があった。撮影機材によって程度が異なり（DJI由来の動画で顕著、Sony由来では軽微）、高解像度・高ビットレートな動画のデコード負荷が原因と考えられる。
  - まず、モーダルを開いた瞬間に`autoPlay`でフルデコードを開始させている点（モーダルの表示アニメーションと重なりカクつきやすい）を解消するため、`autoPlay`を外しユーザー操作で再生開始する形に変更した（TDD: 先にテストを追加しRedを確認してから実装）。
  - あわせて、HEVC(H.265)のプラットフォームハードウェアデコードを有効化するChromiumの起動オプション（`enable-features=PlatformHEVCDecoderSupport`）を試験的に追加したが、ユーザーによる動作確認の結果、体感できる改善が見られなかったため撤回した。
* **各ファイルへの影響と変更内容**:
  - **`src/components/PreviewModal.tsx`**: 動画プレビューの`<video>`要素から`autoPlay`属性を削除。
  - **`src/components/__tests__/PreviewModal.tests.tsx`**: `autoPlay`が付与されないことを検証するテストを追加。
  - `electron/main/main.ts`へのHEVCハードウェアデコード有効化フラグの追加は、効果が確認できなかったため同一作業内で取り消し済み（`electron/main/__tests__/main.tests.ts`のテストも合わせて取り消し）。
  - **仕様書 / README.md**: 変更なし（内部的なパフォーマンス改善のため、UI要件・仕様への影響はない）。
  - `npm run lint`, `npm run typecheck`, `npm run build`, `npm run test:unit`（24ファイル・134テスト成功）で確認済み。
