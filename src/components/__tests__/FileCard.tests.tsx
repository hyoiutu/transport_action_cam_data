...
describe('FileCardに関するテスト', () => {
  test('動画ファイルの場合、video要素とvideoバッジが表示される', () => {
    ...
    // Act
    const { container } = renderWithChakra(<FileCard file={file} onClick={vi.fn()} />);

    // Assert
    // video要素にはaccessible roleが存在しないため、DOM構造の確認にはquerySelectorを用いる
    expect(container.querySelector('video')).not.toBeNull();
    expect(container.querySelector('[data-media-type="video"]')).not.toBeNull();
  });
  ...
  test('画像ファイルの場合、img要素とimageバッジが表示される', () => {
    ...
    expect(container.querySelector('[data-media-type="image"]')).not.toBeNull();
  });
  ...
  test('ファイル名・撮影日・サイズが表示される', () => {
    ...
    // Act
    renderWithChakra(<FileCard file={file} onClick={vi.fn()} />);
  ...
  test('クリックするとonClickが呼ばれる', () => {
    ...
    // Act
    renderWithChakra(<FileCard file={file} onClick={onClick} />);
